...
**Ghosium Browser by Brendigo** is a Windows x64 browser developed as a full-source Ghosium product. The current development version is **0.1.9**.

## Actual Ghosium interface

The images below are generated by GitHub Actions from the checked-in **0.1.9** Ghosium UI and rendered by the pinned Windows Chromium engine. They are real rendered Ghosium-owned surfaces, not design mockups. They intentionally do **not** claim to show the canonical full-source native browser chrome until the controlled full-source build produces that evidence.

### New Tab

<p align="center">
  <img src="docs/images/ghosium-new-tab.png" alt="Ghosium Browser 0.1.9 New Tab" width="100%">
</p>

### Customize Ghosium

<p align="center">
  <img src="docs/images/ghosium-options.png" alt="Ghosium Browser 0.1.9 customization options" width="100%">
</p>

### Ghosium Privacy

<p align="center">
  <img src="docs/images/ghosium-privacy.png" alt="Ghosium Privacy 0.1.9" width="720">
</p>

Screenshot provenance is recorded in `docs/images/GHOSIUM-SCREENSHOTS.json`.
...
A source transformation audit or hosted CI contract is not proof that a production binary exists. **0.1.9 must not be described as a released source-built binary until the controlled Windows compile succeeds for the exact production commit.**
...
Ghosium 0.1.9 defines **38 supported product locales**. English (`en-US`) is the primary/default language and Croatian (`hr`) is required and selectable.
...
`Ghosium-Browser-Setup.exe` is the canonical maintenance package for normal install, `/S /UPDATE` and `/S /UNINSTALL`. `Ghosium-Browser-Portable.exe` is the registry-free portable package and keeps its profile beside the portable runtime. Ghosium does not intentionally distribute a separate public updater or uninstaller executable.
...
- `scripts/build-source-release-installer.ps1` — canonical Setup + Portable packaging/signing;
...
Every source-built release candidate must produce `GHOSIUM-PERFORMANCE.json` from the newly compiled `Ghosium-Browser.exe`. No numerical 0.1.9 performance claim is valid until that evidence exists.

## Production signing

Production `main` builds require the controlled Windows builder to have the configured Brendigo/Ghosium Authenticode signing identity and RFC3161 timestamp endpoint. Browser, proxy, Setup and Portable publisher relationships must validate before production update metadata or release publication can proceed.
...
Internal build intermediates are evidence only; the end-user Windows packages are the canonical Ghosium Setup and Portable executables.
...
The retired wrapper-launcher stable-release path remains excluded. The current Portable package is generated from the same verified Ghosium source stage as Setup and does not restore the retired legacy stable-release architecture.
