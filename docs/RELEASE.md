...
The active development product version is `0.0.3`.

## Current release state

**Ghosium Browser 0.0.3 is a hardening candidate and is not published as the canonical production release.**

Earlier releases and candidates are intentionally distinct from the canonical 0.0.3 full-source production release. Their evidence cannot substitute for the gates below.

The checked-in stable update baseline remains fail-closed.

## Canonical production scope

The canonical 0.0.3 production path carries the current Ghosium branding, native upstream New Tab integration, privacy/stability hardening, `ghost://` WebUI namespace, 38-locale contract, canonical Windows Setup/Portable packaging and fail-closed update/signing/provenance controls. The updater additionally requires unique secure temporary staging, and the release toolchain rejects arbitrary user-writable NSIS PATH resolution.

## Canonical candidate sequence

1. Merge the reviewed 0.0.3 production baseline only after its normal PR contracts pass.
2. Create `ghosium/release/0.0.3` from the exact approved production baseline with exactly `.release/ghosium-v0.0.3.request` containing `ghosium-v0.0.3` when the release-marker contract requires candidate promotion.
...
7. Require production signing and immutable canonical `ghosium-v0.0.3` GitHub publication.
